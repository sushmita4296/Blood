...
class Forgetpass extends CI_Controller {
	function __construct(){
		...
		// }
		$this->load->model('admin_post_model/forgetpassword/forgetpass_model');
	...
	public function index()
	{
		...
		$this->load->view('dashboard/forgetpassword/forgetpass');
		// $this->load->view('provider_dashboard_view/sidebar');
		// $this->load->view('footer');
	}
	public function reset(){
		 $email = $this->input->post('email');
		$result = $this->db->get_where('admin',array('email'=>$email))->row();
		...
		if($result){
			...
			$message = "Please click on password reset link <br><a href='".base_url()."dashboard/forgetpassword/forgetpass/verify/".$token."/".$email."'>Reset Password</a>
			<p>token:".$token."</p>";
			...
			$config = array(
			    ...
			    if($this->email->send())
			    {
				 ...
				 redirect('dashboard/forgetpassword/forgetpass');
	...
	public function verify(){
		// echo
		 ...
		 // echo $token;
		 $this->load->view('header');
		 $this->load->view('dashboard/forgetpassword/recoverpass_form',$data);
	}
