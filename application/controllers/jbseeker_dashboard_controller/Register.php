...
class Register extends CI_Controller {
 ...
 public function __construct()
 {
  ...
  $this->load->model('jbseeker_dashboard_model/register_model');
 }
 ...
 function validation()
 {
  ...
  $this->form_validation->set_rules('email', 'Email Address', 'required|trim|valid_email|is_unique[job_seeker.email]');
...
//////////////extra end
   ...
   if($id > 0)
   {
    ...
    <p>This is email verification mail from Codeigniter Login Register system. For complete registration process and login into system. First you want to verify you email by click this <a href='".base_url()."jbseeker_dashboard_controller/register/verify_email/".$verification_key."'>link</a>.</p>
    ...
    if($this->email->send())
    {
     ...
     redirect('jbseeker_dashboard_controller/register');
 ...
 function verify_email()
 {
  if($this->uri->segment(4))
  {
   ...
   {
    $data['message'] = '<h1 align="center">Your Email has been successfully verified, now you can login from <a href="'.base_url().'Jobseeker/jbseeker">here</a></h1>';
   }
   else
   {
    $data['message'] = '<h1 align="center">Invalid Link</h1>';
   }
   $this->load->view('jbseeker_dashboard_view/email_verification', $data);
  }
 }
