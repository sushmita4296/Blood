...
class Forgetpass extends CI_Controller {
	function __construct(){
		...
		// }
		$this->load->model('jbseeker_dashboard_model/forgetpassword/forgetpass_model');
	...
	public function index()
	{
		...
		$this->load->view('jbseeker_dashboard_view/forgetpassword/forgetpass');
		
	}
	public function reset(){
		 $email = $this->input->post('email');
		$result = $this->db->get_where('job_seeker',array('email'=>$email))->row();
		...
		if($result){
			...
			$message = "Please click on password reset link <br><a href='".base_url()."jbseeker_dashboard_controller/forgetpassword/forgetpass/verify/".$token."/".$email."'>Reset Password</a>
			<p>token:".$token."</p>";
			...
			$config = array(
			    ...
			    if($this->email->send())
			    {
				 ...
				 redirect('jbseeker_dashboard_controller/forgetpassword/forgetpass');
	...
	public function verify(){
		// echo
		 ...
		 // echo $token;
		 $this->load->view('header');
		 $this->load->view('jbseeker_dashboard_view/forgetpassword/recoverpass_form',$data);
	}
